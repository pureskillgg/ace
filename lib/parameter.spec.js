...
import { Parameter, ParameterParseError, ParameterValidateError } from './parameter.js'
...
test('get: returns value from provider', async (t) => {
  ...
  td.when(Provider.prototype.get(key, undefined)).thenResolve(value)
...
test('get: returns fallback value from provider', async (t) => {
  const key = 'mock-key'
  const fallback = 'mock-fallback'

  const Provider = td.constructor(['get'])

  td.when(Provider.prototype.get(key, fallback)).thenResolve(fallback)

  const parameter = new Parameter({ key, fallback, Provider })
  parameter.initProvider()
  const data = await parameter.get()
  t.is(data, fallback)
})

test('get: returns fallback value from provider with override fallback', async (t) => {
  const key = 'mock-key'
  const fallback = 'mock-fallback'
  ...
  const parameter = new Parameter({ key, fallback: 'other-fallback', Provider })
  parameter.initProvider()
  const data = await parameter.get(key, fallback)
  t.is(data, fallback)
})
...
test('get: throws provider error', async (t) => {
  ...
  td.when(Provider.prototype.get(key, undefined)).thenReject(err)
...
test('get: throws error on parse error', async (t) => {
  ...
  const parameter = new Parameter({
    key,
    Provider,
    parser: (value) => { throw err }
  })
  parameter.initProvider()

  const error = await t.throwsAsync(() => parameter.get(), {
    instanceOf: ParameterParseError
  })
  t.is(error.key, key)
  t.is(error.data, 'mock-value')
...
test('ParameterValidateError', (t) => {
  const key = 'mock-key'
  const data = 'mock-value'
  const err = new ParameterValidateError(key, data)
  t.like(err, {
    name: 'ParameterValidateError',
...
test('ParameterParseError', (t) => {
  const key = 'mock-key'
  const data = 'mock-value'
  const error = new Error('foo')
  const err = new ParameterParseError(key, data, error)
  t.like(err, {
