...
test('get: returns fallback if no data for parameter', async (t) => {
  const ssmClient = td.object(['send'])
  const provider = new SsmProvider({ ssmClient })
  const parameterName = 'parameter-name'
  td.when(ssmClient.send(td.matchers.anything())).thenResolve(null)
  const data = await provider.get(parameterName, 'mock-fallback')
  t.is(data, 'mock-fallback')
})

test('get: returns fallback error if no value for parameter', async (t) => {
  const ssmClient = td.object(['send'])
  const provider = new SsmProvider({ ssmClient })
  const parameterName = 'parameter-name'
