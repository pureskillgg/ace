...
test('get: returns parameter fallback if value not found', async (t) => {
  const parameterName = 'parameter-name'
  const parameterFallback = 'parameter-fallback'
  const localParameters = {}
  const provider = new LocalProvider({ localParameters })
  const data = await provider.get(parameterName, parameterFallback)
  t.is(data, parameterFallback)
})
